...
router.get('/penghuni', auth, adminCheck, async (req, res) => {
  try {
    ...
    const penghuniList = await User.find({ role: 'USER' }).select('username _id'); // Sesuaikan field yang diperlukan
...
router.get('/payment-status', auth, adminCheck, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error fetching payment status counts', error });
  }
});
