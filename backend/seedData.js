...
const calculateTotalBill = (months, services = []) => {
  const basePrice = months === 3 ? PRICES.ROOM.THREE_MONTHS : PRICES.ROOM.SIX_MONTHS;
  return services.reduce((total, service) => {
    return total + PRICES[service][months === 3 ? 'THREE_MONTHS' : 'SIX_MONTHS'];
  }, basePrice);
};
...
const seedData = async () => {
  try {
    ...
    const rooms = await Promise.all(
      Array.from({ length: 10 }, (_, i) => Room.create({
        number: `${i + 1}`,
        is_occupied: i < 2,
    ...
    if (error.name === 'MongoNetworkError') {
      console.log('Please check your MongoDB connection and make sure MongoDB is running');
    }
  } finally {
    try {
