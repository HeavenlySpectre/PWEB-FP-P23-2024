      ...
      <nav class="top-nav">
        <div class="container nav-content">
          ...
          <div class="nav-links">
            ...
            <router-link to="/admin/laporan/fasilitas" class="active">Laporan Fasilitas</router-link>
        ...
        <div class="container">
          <div class="card">
            ...
            <div v-else-if="error" class="error-state">
              <p>{{ error }}</p>
              <button @click="fetchReports" class="btn btn-primary">Coba Lagi</button>
            </div>
    ...
    setup() {
      ...
      const filteredReports = computed(() => {
        return reports.value.filter(report => {
          const matchesSearch = 
            (report.message && report.message.toLowerCase().includes(searchQuery.value.toLowerCase())) ||
            (report.user && report.user.username.toLowerCase().includes(searchQuery.value.toLowerCase())) ||
            (report.room_number && report.room_number.toString().includes(searchQuery.value));
      ...
      const fetchReports = async () => {
        try {
          loading.value = true;
          error.value = null;
          const response = await axios.get('http://localhost:5000/api/reports/facility', {
            headers: {
      ...
      const updateStatus = async (reportId, newStatus) => {
        try {
          await axios.patch(
            `http://localhost:5000/api/reports/facility/${reportId}`,
            { status: newStatus },
            {
              headers: {
                Authorization: `Bearer ${authStore.token}`
