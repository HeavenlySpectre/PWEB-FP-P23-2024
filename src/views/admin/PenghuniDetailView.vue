          ...
          <div class="content-section">
            <div class="card">
              ...
              <div v-else-if="error" class="error-state">
                <p>{{ error }}</p>
                <button @click="fetchUserDetails" class="btn btn-primary">Coba Lagi</button>
              </div>
              ...
              <div v-else class="payment-details">
                <div class="section">
                  ...
                  <div class="info-row">
                    ...
                    <span>{{ formatPaymentMethod(userDetails?.payment?.payment_method) }}</span>
                  </div>
                </div>
                ...
                <div class="section total">
                  <div class="info-row">
                    ...
                    <span class="total-amount">{{ formatCurrency(userDetails?.payment?.total_bill) }}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div v-if="showConfirmDialog" class="modal-overlay">
        <div class="modal-content">
          ...
          <div class="modal-actions">
            <button @click="removePenghuni" class="btn btn-danger">Ya, Hapus</button>
            <button @click="showConfirmDialog = false" class="btn btn-secondary">Batal</button>
          </div>
        </div>
      </div>
    </div>
    ...
    setup() {
      ...
      const fetchUserDetails = async () => {
        ...
        try {
          ...
          const response = await axios.get(`http://localhost:5000/api/admin/penghuni/${userId.value}`, {
            headers: {
          ...
          if (err.response) {
            ...
            error.value = 'Tidak ada respons dari server. Periksa koneksi internet Anda.';
          } else {
      ...
      const getPaymentStatusClass = () => {
        ...
        return paymentStatusClassMap[userDetails.value.payment.status] || 'unknown';
      };
  
      // Fungsi untuk mendapatkan label status pembayaran
      const getPaymentStatusLabel = () => {
        if (!userDetails.value) return '';
        return paymentStatusMap[userDetails.value.payment.status] || 'Status Tidak Diketahui';
      };
      ...
      const removePenghuni = async () => {
        try {
          await axios.delete(`http://localhost:5000/api/admin/penghuni/${userId.value}`, {
            headers: {
              Authorization: `Bearer ${authStore.token}`
            }
          });
